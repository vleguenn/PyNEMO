...
class Coord:
    ...
    def __init__(self, fname, bdy_ind):
        ...
        self.var_nb_ij_id = self._build_dict(['i', 'j'], 
                                 ['nb', 'int', 'unitless', 0]) # i4
        self.var_nb_r_id = self._build_dict(['r'], 
                                 ['nb', 'int', 'unitless', 0]) # i4
        self.var_g_lamphi_id = self._build_dict(['lam', 'phi'], #f4
                                 ['g', 'float', 'degrees_east', 'longitude'])
        self.var_e_12_id = self._build_dict(['1', '2'], #f4
                                 ['e', 'float', 'metres', 'scale factor'])
        ...
        #self.populate('/work/jofa/grid_low_res/mesh_hgr_zps.nc')
        #self.closeme()   
        # Leave Define Mode
        #self.closeme()
...
# # # # # # # # # 
    ...
    def openfile(self, fname, mode, clob, version):
        return Dataset(fname, mode, clobber=clob, format=version)
    ...
    def _create_dims(self):
        ...
        ret['xb']['t'] = self.ncid.createDimension('xbT', 
                                                   len(self.bdy_ind['t'][0]))
        ret['xb']['u'] = self.ncid.createDimension('xbU', 
                                                   len(self.bdy_ind['u'][0]))
        ret['xb']['v'] = self.ncid.createDimension('xbV', 
                                                   len(self.bdy_ind['v'][0]))
        ret['xb']['f'] = self.ncid.createDimension('xbF', 
                                                   len(self.bdy_ind['f'][0]))
        ...
        for ind in vardic:
            ...
            for dim in vardic[ind]:
                if nc is not None:
                    ...
                    data = data.squeeze()
                    data = data[(self.bdy_ind[ind][0][:,1]),
                                (self.bdy_ind[ind][0][:,0])]
                elif len(vardic[ind]) == 1:
                    data = self.bdy_ind[ind][1][:]
                else:
                    print 'here'
                    data = self.bdy_ind[ind][0][:,x]
                    x = 1
